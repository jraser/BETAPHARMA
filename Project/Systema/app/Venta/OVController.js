var OVController = function ($rootScope, $scope, $state, $stateParams, $http, $modal) {
    $scope.parameters = {
    ...
    $scope.ActualizarOden = function () {
        $http({
            ...
            url: 'api/TabGenWS',
            params: {
                DescripcionTab: 'DetalleOrdenDeVenta',
            }
        }).success(function (response) {
    ...
    $scope.editDetalle2 = function (Id, CodProducto, DespProducto, PUnitario, ICantidad, IDescuento, IMonto, Empresa) {
        var modalInstance = $modal.open({
    ...
    $scope.addItemModific = function () {

        var modalInstance = $modal.open({
            templateUrl: '/Stock/AddStock',
            controller: 'StockAddController',
            resolve: {
                scope: function () {
                    return $scope;
    ...
    $scope.getdetalle = function (lista) {
        ...
        $.each($scope.ProductoList, function (index, value) {
            ...
            value.Cantidad = 1;
            value.PorcDscto = 0;
            value.Monto = (value.PrecioUnitario * value.Cantidad) - (((value.PrecioUnitario * value.Cantidad) * value.Descuento)/100);
        });
    ...
    $scope.Calcular = function () {
        ...
        $.each($scope.ProductoList, function (index, value) {
            ...
            value.Monto = (NumberDecimal(value.PrecioUnitario) * NumberInt(value.Cantidad)) - (((NumberDecimal(value.PrecioUnitario) * NumberInt(value.Cantidad)) * value.Descuento)/100);
...
OVController.$inject = ['$rootScope', '$scope', '$state', '$stateParams', '$http', '$modal'];