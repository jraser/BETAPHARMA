var OCAddController = function ($rootScope, $scope, $modalInstance, $http, $modal, ID_Socio, scope) {
    ...
    $scope.ListOrden = function () {
        ...
        $http({
            ...
            $scope.OrdenCompraListPorFecha = response;
    ...
    $scope.Select = function (item) {
        $.each($scope.OrdenCompraListPorFecha, function (index, value) {
    ...
    function modeAction() {
        if ($scope.Item.ID_Orden != 0) {
            ...
            scope.getOrdenCompra($scope.Item);
            $modalInstance.dismiss('cancel');
...
OCAddController.$inject = ['$rootScope', '$scope', '$modalInstance', '$http', '$modal', 'ID_Socio', 'scope'];
